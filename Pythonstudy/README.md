...
| 历史记录 | 每次考试自动保存日期、难度、题量、得分、正确率、错题数、用时 |
| 考试计时 | 自动记录考试用时，展示几分几秒和平均每题用时 |
| 题目管理 | 交互式单条添加 / 文件批量导入 / 题库统计（含答案分布） |
...
│   ├── python_easy.txt      # 简单题（40 道）
│   ├── python_medium.txt    # 中等题（40 道）
│   └── python_hard.txt      # 困难题（40 道）
...
题库为 UTF-8 编码的纯文本文件，**每行一道题**，8 个字段用竖线 `|` 分隔：

```
难度|题目内容|选项A|选项B|选项C|选项D|正确答案|题目ID
```
...
| 8 | 题目ID | `E001` / `M001` / `H001` 等 | easy用E前缀，medium用M前缀，hard用H前缀，后接3位序号 |

### 示例

```
easy|Python 中定义函数用哪个关键字？|function|def|func|define|B|E001
medium|列表推导式 [x*2 for x in range(3)] 的结果是？|[0,1,2]|[0,2,4]|[2,4,6]|报错|B|M001
hard|Python 中 GIL 的主要影响是？|多进程无法并行|多线程CPU密集型无法真正并行|单线程变慢|内存泄漏|B|H001
```

### 规则

- 以 `#` 开头的行为**注释**，空行会被忽略
- 一行必须恰好 **8 个字段**，否则程序警告并跳过
- `questions/` 目录下可放**多个** `.txt` 文件，主程序自动合并
- 保存编码必须为 **UTF-8**，否则中文乱码
- 题目ID在同一难度内必须唯一，用于错题去重
...
[
  {
    ...
    "wrong_count": 2,
    "last_wrong_date": "2026-08-30 20:05:00"
  }
]
```
...
| 版本 | 日期 | 主要变更 |
|---|---|---|
| v2.1.0 | 2026-08-30 | 题库扩充至120道（每难度40道）、题库格式升级为8字段（加ID）、答案分布均匀化、考试计时功能、错题去重改用ID、历史记录ID改用毫秒时间戳、原子写入+备份、题库不足保护、MIT许可证 |
...
本项目基于 [MIT License](LICENSE) 开源协议发布。Copyright (c) 2026 SUN-yc13。
